/***************************************************************
 * Author: Thomas Kramer * Purpose: To convert MorseCode to English and visversa
...
//If a match add the match to string that at the end of loop will form the converted value.
        for (int i = 0; i < inputtedEnglish.length(); i++)
        {
            for (int j = 0; j < MORSETRANSLATIONARRAY.length; j++)
            {
                if (Character.toString(inputtedEnglish.charAt(i)).toUpperCase().equals(MORSETRANSLATIONARRAY[j][0]))
                {
                    String temp = MORSETRANSLATIONARRAY[j][1];
...
//If a match is found add it to string that will form the translated word
        ...
        {
            for (int i = 0; i < MORSETRANSLATIONARRAY.length; i++)
            {
